...
#
# 规格: 在完成前让 `npm run check`（= typecheck && test，经 npm workspaces）
...
# 若不是 supadevops monorepo(root package.json 无 check script)则什么都不做。
...
{
  echo "supadevops: 回归确认失败(npm run check)。"
