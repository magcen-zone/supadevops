---
name: supa-tdd
description: supadevops の中核。Next.js(App Router)/ Expo(React Native)の npm workspaces + turborepo モノレポで、実装より先に JSDoc で契約を確定し、5フェーズ(Plan→Test→Implement→受入→Finish)とヒューマンゲートで進める JS+JSDoc 契約優先 + TDD 開発フローを課す。新機能・バグ修正・リファクタを supadevops 規律で進めるときに使う。Superpowers を補強する。
---

# supadevops — JSDoc 契約優先 + TDD 開発フロー

実装より先に **JSDoc 契約**を確定し、テストと実装をそこへ収束させる。**順序を飛ばさない。** 規約・記入例の正典は本プラグイン同梱の `README.md`(§1–§3.7)。本スキルはその要点と適用手順。

## 前提(§1.1)
- **JavaScript + JSDoc**(TypeScript 構文は書かない)。型はすべて JSDoc。`.d.ts` は作らない。型検査は `tsc -p jsconfig.json --noEmit`。
- **ESM**(`"type":"module"`)。
- 対象は **npm workspaces + turborepo のモノレポ**。`app/next-<名>`(Next.js `src/app`)・`app/expo-<名>`(Expo)・`package/*`(共有ライブラリ=プラットフォーム非依存ロジック/型のみ)。無ければ先に **`/supa-init`**。
