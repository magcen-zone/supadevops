...
```
SUPA_STARTER_REF = magcen-zone/supa-starter#v0.2.0
```
...
```bash
npx --yes degit "magcen-zone/supa-starter#v0.2.0" .
```

degit 取得该 tag 的快照（**无 `.git` / 无 `node_modules`**）。失败时回退到浅克隆：

```bash
git clone --depth 1 --branch v0.2.0 git@github.com:magcen-zone/supa-starter.git .supa-tmp \
