...
```
SUPA_STARTER_REF = magcen-zone/supa-starter#v0.3.0
```
...
- 确认四个名字（next / expo / gas / lib）。

## 1. 取得模板（degit，固定 tag）

```bash
npx --yes degit "magcen-zone/supa-starter#v0.3.0" .
```

degit 取得该 tag 的快照（**无 `.git` / 无 `node_modules`**）。失败时回退到浅克隆：

```bash
git clone --depth 1 --branch v0.3.0 git@github.com:magcen-zone/supa-starter.git .supa-tmp \
  && rm -rf .supa-tmp/.git && cp -R .supa-tmp/. . && rm -rf .supa-tmp
```

## 2. 确定性重命名

用插件内置脚本把占位名 `next-app` / `expo-app` / `gas-app` / `core`（npm 名 `@app/core`）替换为项目名 —— 目录名 + 文件内 token + `app.json` 标识 + **锁文件中的 workspace 项**，并删除模板的 `README.md` / `LICENSE`：

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/rename-starter.mjs" . --next <next-名> --expo <expo-名> --gas <gas-名> --lib <lib-名>
```
